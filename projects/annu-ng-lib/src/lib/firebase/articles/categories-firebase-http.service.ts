...
import {
  PageCategories,
  PageCategoryGroup,
} from './articles-firebase.interface';
...
export class CategoriesFirebaseHttpService {
    ...
    return new Promise((resolve, reject) => {
      if (!id) {
        ...
        const httpSubscription = this.http.get(url).subscribe({
          next: (c: any) => {
            const category: Category = this.firestoreParser.parse(
              c
            ) as Category;
  ...
  public async runQueryByConfig(
    ...
    return new Promise((resolve, reject) => {
      ...
      const httpSubscription = this.http
        ...
        .subscribe({
          next: (cats: any) => {
            const categories: Array<Category> = this.firestoreParser.parse(
              cats
            ) as Array<Category>;
            httpSubscription.unsubscribe();
            resolve(categories);
  ...
  public async getAllLiveCategoriesWithOnePageShallowArticles(
    ...
    try {
      const categories = await this.runQueryByConfig(categoriesQueryConfig);
      return await this.articlesHttp.getLiveShallowArticlesOfCategories(
        categories,
        pageSize,
        startPage,
        isForwardDir
