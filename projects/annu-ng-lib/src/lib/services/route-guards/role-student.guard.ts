...
@Injectable()
export class RoleStudentGuard implements CanActivate, CanActivateChild {
  ...
  public async checkUserHasRole(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<boolean | UrlTree> {
    ...
    const isInRole = await this.authFireSvc.currentUserHasRole(FIREBASE_AUTH_ROLES.STUDENT);
